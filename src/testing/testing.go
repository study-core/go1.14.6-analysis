...
var (
	...
	outputDir            *string
	chatty               *bool
	count                *uint		// 记录目前 "go test" 时, 有多少个 测试用例 需要执行
	coverProfile         *string
	matchList            *string
	match                *string
	memProfile           *string
...
// todo testing.T 和 testing.B 的基础类型
//
// common holds the elements common between T and B and
// captures common methods such as Errorf.
//
// common 持有 T 和 B 之间的公共元素，并捕获诸如 Errorf 之类的公共方法
type common struct {
	...
	// 写入 文件/行  (file/line) 信息时要被 跳过的功能
	//
	// 标记当前为函数为help函数， 其中打印的日志， 在记录日志时不会显示其 文件名 及 行号
	...
	// 健谈标志的副本 ??
	//
	// 对应命令行中的 -v 参数， 默认为false， true则打印更多详细日志
...
// This function must be called with c.mu held.
//
// todo 装饰 testing 的日志格式
//
// `decorate()` 前缀为 调用站点的 文件 和 行，并在需要时插入最后的 换行符 和 缩进 空格 以 进行格式化。
//  必须在保持c.mu的情况下调用此函数。
...
//
//
// `flushToParent()`  首先使用给定 的格式 和 参数写入 标头 ，然后将c.output写至 父级
...
type T struct {
	...
	// 是否启动 并行测试      如果测试中执行了t.Parallel()， 则此值为true
	isParallel bool
	...
	if c.done {
		...
		for parent := c.parent; parent != nil; parent = parent.parent {
			...
			if !parent.done {
				...
				// 内部记录日志入口， 日志会统一记录到common.output切片中， 测试结束时再统一打印出来.
				//
		...
		if c.chatty {   // 单元测试中记录的日志只有在执行失败或指定了 -v 参数才会打印， 否则不会打印
...
// todo 标记当前函数为 help 函数
// 		所谓 help 函数， 即其中打印的日志， 不记录 help 函数的 `函数名` 及 `行号`， 而是记录 上一层函数的函数名和行号
//
//
...
func tRunner(t *T, fn func(t *T)) {
	...
	defer func() {
		...
		if len(t.sub) > 0 {
			...
			// 如果当前测试非并发模式， 则等待并发执行， 类似于测试函数中执行t.Parallel()
			if !t.isParallel {
		...
		t.signal <- signal   // 向调度者发送结束信号  // 向父测试发送结束信号， 以便结束Run()
	}()
	defer func() {
		if len(t.sub) == 0 {
...
func (t *T) Run(name string, f func(t *T)) bool {
	...
	if !<-t.signal {    // 阻塞等待子测试结束信号， 子测试要么执行结束， 要么以 Parallel() 执行。 如果信号为 "false"， 说明出现异常退出
...
type testContext struct {
	...
	// 互斥锁， 用于控制testContext成员的互斥访问
	mu sync.Mutex

	// 用于通知测试 可以并发执行的控制管道， 测试并发达到最大限制时， 需要阻塞等待该 chan 的 通知事件
	//
	...
	// 最大并发数， 默认为系统CPU数， 可以通过参数 -parallel n 指定
	//
...
func (c *testContext) waitParallel() {
	...
	// 小细节， 阻塞等待后面并没有累加c.running， 因为其他并发的测试结束后也不会递减c.running，
	// 所以这里阻塞返回时也不用累加，
	// 一个测试结束， 随即另一个测试开始， c.running个数没有变化
}

// 释放一个信号， 用于启动其他等待并发测试的函数
...
func (t *T) report() {
	...
	} else if t.chatty {  // 对应命令行中的 -v 参数， 默认为false， true则打印更多详细日志
...
// it is part of the implementation of the "go test" command.
//
// todo 执行 "go test" 时的入口
//
// `RunTests()` 是一个内部函数，但由于是交叉打包而被导出
//				这是  "go test"  命令的实现的一部分
